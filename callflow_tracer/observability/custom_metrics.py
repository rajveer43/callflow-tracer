...
class SLAMonitor:
    ...
    def record_metric(self, metric_name: str, value: float, timestamp: float = None):
        ...
        for condition in self.conditions.get(metric_name, []):
            ...
            if (
                condition.dynamic and len(self.metric_history[metric_name]) > 10
            ):  # Need some history
                historical_values = [
                    m["value"] for m in self.metric_history[metric_name][:-1]
                ]
        ...
        for metric_name, conditions in self.conditions.items():
            ...
            for condition in conditions:
                ...
                total = len(
                    [
                        m
                        for m in self.metric_history.get(metric_name, [])
                ...
                report[metric_name].append(
                    {
                        ...
                        "last_violation": (
                            max(v["timestamp"] for v in violations)
                            if violations
                            else None
                        ),
    ...
    def get_violations(
        self, metric_name: Optional[str] = None, time_window: Optional[int] = None
...
class SLO:
    ...
    def compute(self, tags: Optional[Dict[str, str]] = None) -> Dict[str, Any]:
        if self.sli_type == "availability":
            value = SLI.availability(
                self.metric_name, time_window=self.time_window, tags=tags, **self.params
            )
            ...
            value = SLI.latency_target_ratio(
                self.metric_name, time_window=self.time_window, tags=tags, **self.params
            )
        else:
            value = None
...
class ErrorBudgetTracker:
    ...
    def compute_budget(self, tags: Optional[Dict[str, str]] = None) -> Dict[str, Any]:
        ...
        consumed = (
            min(1.0, actual_error / allowed_error)
            if allowed_error > 0
            else (1.0 if actual_error > 0 else 0.0)
        )
