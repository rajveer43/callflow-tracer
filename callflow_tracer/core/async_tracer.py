...
class AsyncCallGraph(CallGraph):
    ...
    def track_concurrent_start(self, task_name: str):
        ...
        if (
            self.concurrent_tasks[task_name]["count"]
            > self.concurrent_tasks[task_name]["max"]
        ):
            self.concurrent_tasks[task_name]["max"] = self.concurrent_tasks[task_name][
                "count"
            ]
...
def trace_async(func: Callable) -> Callable:
    ...
    if not asyncio.iscoroutinefunction(func):
        raise TypeError(
            f"{func.__name__} is not an async function. Use @trace for sync functions."
        )
...
@asynccontextmanager
async def trace_scope_async(
    output_file: Optional[str] = None, include_args: bool = False
):
    """
...
def get_async_stats(graph: AsyncCallGraph) -> dict:
    ...
    max_concurrent = max(
        (task["max"] for task in graph.concurrent_tasks.values()), default=0
    )
    ...
    return {
        ...
        "efficiency": round(
            (total_active_time / total_async_time * 100) if total_async_time > 0 else 0,
            2,
        ),
