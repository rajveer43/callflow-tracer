...
from .tracer import CallTracer, get_current_graph, trace_scope
...
class HTTPInstrumentor(AutoInstrumentor):
    ...
    def _record_http_call(
        ...
        if graph.tracer and graph.tracer.call_stack:
            caller_name = (
                graph.tracer.call_stack[-1] if graph.tracer.call_stack else caller_name
            )
