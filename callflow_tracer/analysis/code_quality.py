...
class ComplexityAnalyzer:
    ...
    def analyze_function(
        self, func_node: ast.FunctionDef, module: str = ""
    ) -> ComplexityMetrics:
...
class MaintainabilityAnalyzer:
    ...
    def analyze_function(
        ...
        mi = self._calculate_maintainability_index(halstead["volume"], complexity, loc)
    ...
    def _calculate_halstead_metrics(self, node: ast.AST) -> Dict[str, float]:
        ...
        for child in ast.walk(node):
            ...
            if isinstance(
                ...
                child, (ast.Eq, ast.NotEq, ast.Lt, ast.LtE, ast.Gt, ast.GtE)
            ):
                operators.add(type(child).__name__)
                operator_count += 1
    ...
    def _calculate_maintainability_index(
        self, volume: float, complexity: int, loc: int
    ) -> float:
    ...
    def analyze_from_metrics(
        ...
        for comp in complexity_metrics:
            ...
            if comp.cyclomatic_complexity > 20:
                issues.append(
                    f"Very high cyclomatic complexity ({comp.cyclomatic_complexity})"
                )
                debt_score += 30
            elif comp.cyclomatic_complexity > 10:
                issues.append(
                    f"High cyclomatic complexity ({comp.cyclomatic_complexity})"
                )
                debt_score += 15

            if comp.cognitive_complexity > 15:
                issues.append(
                    f"High cognitive complexity ({comp.cognitive_complexity})"
                )
                debt_score += 20
            ...
            if maint:
                if maint.maintainability_index < 20:
                    issues.append(
                        f"Critical maintainability ({maint.maintainability_index:.1f})"
                    )
                    debt_score += 40
                elif maint.maintainability_index < 40:
                    issues.append(
                        f"Poor maintainability ({maint.maintainability_index:.1f})"
                    )
                    debt_score += 25
...
class QualityTrendAnalyzer:
    ...
    def add_snapshot(
        ...
        avg_complexity = (
            sum(m.cyclomatic_complexity for m in complexity_metrics)
            / len(complexity_metrics)
            if complexity_metrics
            else 0
        )
        avg_maintainability = (
            sum(m.maintainability_index for m in maintainability_metrics)
    ...
    def analyze_trends(self) -> Dict[str, Any]:
        ...
        maintainability_trend = self._calculate_trend(
            [t.average_maintainability for t in recent]
        )
...
def analyze_codebase(directory: str, file_pattern: str = "*.py") -> Dict[str, Any]:
    ...
    debt_indicators = debt_analyzer.analyze_from_metrics(
        all_complexity, all_maintainability
    )
    ...
    return {
        ...
        "summary": {
            ...
            "average_complexity": (
                sum(m.cyclomatic_complexity for m in all_complexity)
                / len(all_complexity)
                if all_complexity
                else 0
            ),
            "average_maintainability": (
                sum(m.maintainability_index for m in all_maintainability)
                / len(all_maintainability)
                if all_maintainability
                else 0
            ),
            "total_debt_score": sum(d.debt_score for d in debt_indicators),
            "critical_issues": len(
                [d for d in debt_indicators if d.severity == "Critical"]
            ),
