...
class BaselineLearner:
    ...
    def add_sample(self, metric_name: str, value: float) -> None:
        ...
        if len(baseline.samples) >= self.min_samples:
            ...
            baseline.std = (
                statistics.stdev(baseline.samples) if len(baseline.samples) > 1 else 0.0
            )
...
class AnomalyDetector:
    ...
    def __init__(
        ...
        self.drift_windows: Dict[str, deque] = defaultdict(
    ...
    def _generate_description(
        self, metric_name: str, value: float, baseline: BaselineStats, z_score: float
    ) -> str:
        ...
        return (
            f"{metric_name} is {direction} than normal by {diff_percent:.1f}% "
            f"(value: {value:.3f}, baseline: {baseline.mean:.3f}, "
            f"z-score: {z_score:.2f})"
        )

    def _check_drift(
        self, metric_name: str, value: float, baseline: BaselineStats
    ) -> None:
...
def analyze_function_duration(
    function_name: str, duration: float, context: Optional[Dict[str, Any]] = None
) -> Optional[AnomalyAlert]:
...
def analyze_custom_metric(
    metric_name: str, value: float, context: Optional[Dict[str, Any]] = None
) -> Optional[AnomalyAlert]:
