    ...
    def analyze_leaks(self):
        ...
        if len(self.growth_patterns) >= 2:
            consistent_growth = all(
                p["memory_growth"] > 0 for p in self.growth_patterns[-3:]
            )

            if consistent_growth:
                avg_growth_rate = (
                    sum(p["growth_rate"] for p in self.growth_patterns[-3:]) / 3
                )
...
@contextmanager
def detect_leaks(
    output_file: Optional[str] = None, snapshot_interval: Optional[float] = None
):
    """
...
def get_memory_growth(
    interval: float = 1.0, iterations: int = 5
) -> List[Dict[str, Any]]:
    """
