...
class QueryEngine:
    ...
    def query(
        self, graph: CallGraph, question: str, use_llm: bool = True
    ) -> Dict[str, Any]:
        """
    ...
    def _query_slow_functions(
        self, graph: CallGraph, question: str
    ) -> List[Dict[str, Any]]:
        ...
        sorted_nodes = sorted(
            graph.nodes.values(), key=lambda n: n.total_time, reverse=True
        )[:limit]
        ...
        return [
            {
                ...
                "percentage": round(
                    (node.total_time / total_time * 100) if total_time > 0 else 0, 1
                ),
            }
            for node in sorted_nodes
        ]

    def _query_fast_functions(
        self, graph: CallGraph, question: str
    ) -> List[Dict[str, Any]]:
        ...
        sorted_nodes = sorted(graph.nodes.values(), key=lambda n: n.total_time)[:limit]
    ...
    def _query_most_called(
        ...
        sorted_nodes = sorted(
            graph.nodes.values(), key=lambda n: n.call_count, reverse=True
        )[:limit]
    ...
    def _query_least_called(
        ...
        sorted_nodes = sorted(graph.nodes.values(), key=lambda n: n.call_count)[:limit]
    ...
    def _query_recursive_functions(
        self, graph: CallGraph, question: str
    ) -> List[Dict[str, Any]]:
    ...
    def _query_by_module(self, graph: CallGraph, question: str) -> List[Dict[str, Any]]:
        ...
        matching_nodes = [
            node
            for node in graph.nodes.values()
            if module_name.lower() in node.module.lower()
            or module_name.lower() in node.full_name.lower()
        ]
    ...
    def _query_call_count(self, graph: CallGraph, question: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_calls_per_function": (
                round(total_calls / len(graph.nodes), 2) if graph.nodes else 0
            ),
        }
    ...
    def _query_total_time(self, graph: CallGraph, question: str) -> Dict[str, Any]:
        ...
        return {"total_time": round(total_time, 3), "total_functions": len(graph.nodes)}
        ...
        if isinstance(data, list):
            ...
            for i, item in enumerate(data, 1):
                if "function" in item:
                    ...
                    calls_info = (
                        f"({item['call_count']} calls)" if "call_count" in item else ""
                    )
                    pct_info = (
                        f"[{item['percentage']}%]" if "percentage" in item else ""
                    )

                    lines.append(
                        f"{i}. {func} - {time_info} {calls_info} {pct_info}".strip()
                    )
