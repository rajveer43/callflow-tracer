...
class AutoFixer:
    ...
    def generate_fixes(
        ...
        fixes.sort(
            key=lambda x: (x["confidence"], x["severity"] != "critical"), reverse=True
        )
    ...
    def _fix_n_plus_one(
        self, issue: Dict[str, Any], before_code: str
    ) -> Optional[CodeFix]:
