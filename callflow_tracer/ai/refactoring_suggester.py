...
class RefactoringSuggester:
    ...
    def suggest_refactoring(
        self, graph: Dict[str, Any], include_code_examples: bool = True
    ) -> List[Dict[str, Any]]:
        """
        ...
        suggestions.sort(
            key=lambda x: (x["priority"], x["impact"] != "low"), reverse=True
        )

        return [
            asdict(s) if isinstance(s, RefactoringSuggestion) else s
            for s in suggestions
        ]

    def _analyze_node(
        self, node: Dict[str, Any], include_code_examples: bool
        ...
        if total_time > 1.0:
            suggestion = RefactoringSuggestion(
                ...
                code_example=(
                    self._get_long_function_example() if include_code_examples else ""
                ),
                impact="high",
                effort="medium",
                priority=5,
        ...
        if call_count > 100:
            suggestion = RefactoringSuggestion(
                ...
                code_example=(
                    self._get_caching_example() if include_code_examples else ""
                ),
                impact="high",
                effort="low",
                priority=4,
        ...
        if call_count > 10 and total_time > 0.1:
            ...
            if avg_time > 0.01:  # Each call takes >10ms
                suggestion = RefactoringSuggestion(
                    ...
                    code_example=(
                        self._get_n_plus_one_example() if include_code_examples else ""
                    ),
                    impact="high",
                    effort="medium",
                    priority=5,
    ...
    def _analyze_structure(
        self, nodes: Dict[str, Dict[str, Any]], include_code_examples: bool
    ) -> List[RefactoringSuggestion]:
        ...
        for coupling in tight_couplings:
            suggestion = RefactoringSuggestion(
                ...
                code_example=(
                    self._get_decoupling_example() if include_code_examples else ""
                ),
                impact="medium",
                effort="high",
                priority=3,
