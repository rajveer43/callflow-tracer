...
class AnomalyDetector:
    ...
    def __init__(
        self, baseline_graphs: Optional[List] = None, sensitivity: float = 2.0
    ):
        """
    ...
    def _detect_time_anomalies(self, nodes: Dict) -> List[Dict[str, Any]]:
        ...
        for node_id, node in nodes.items():
            ...
            if node["total_time"] > threshold:
                z_score = (
                    (node["total_time"] - mean_time) / std_time if std_time > 0 else 0
                )
                ...
                if self.baseline_stats:
                    ...
                    if func_key in self.baseline_stats:
                        ...
                        baseline_threshold = baseline["mean_time"] + (
                            self.sensitivity * baseline["std_time"]
                        )
                        if node["total_time"] > baseline_threshold:
                            anomaly["baseline_deviation"] = (
                                node["total_time"] - baseline["mean_time"]
                            )
                            anomaly[
                                "description"
                            ] += f" (baseline: {baseline['mean_time']:.3f}s)"
    ...
    def _detect_frequency_anomalies(self, nodes: Dict) -> List[Dict[str, Any]]:
        ...
        for node_id, node in nodes.items():
            if node["call_count"] > threshold:
                z_score = (
                    (node["call_count"] - mean_count) / std_count
                    if std_count > 0
                    else 0
                )
    ...
    def _generate_recommendations(self, anomalies: Dict) -> List[str]:
        ...
        n_plus_one = [
            a
            for a in anomalies["pattern_anomalies"]
            if a.get("subtype") == "n_plus_one"
        ]
        if n_plus_one:
            recommendations.append(
