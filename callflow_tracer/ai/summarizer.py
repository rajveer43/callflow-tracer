...
class TraceSummarizer:
    ...
    def summarize(
        ...
        summary_text = self._generate_summary(
            stats, bottlenecks, include_recommendations
        )
    ...
    def _extract_stats(self, graph: CallGraph) -> Dict[str, Any]:
        ...
        most_called = max(
            graph.nodes.values(), key=lambda n: n.call_count, default=None
        )
        ...
        return {
            ...
            "slowest_function": {
                ...
                "percentage": round(
                    (
                        (slowest.total_time / total_time * 100)
    ...
    def _calculate_depth(self, graph: CallGraph) -> int:
        ...
        def dfs(node, visited):
            ...
            visited.add(node)
            max_child_depth = max(
                (dfs(child, visited.copy()) for child in adj[node]), default=0
            )
    ...
    def _identify_bottlenecks(
        self, graph: CallGraph, max_count: int
    ) -> List[Dict[str, Any]]:
    ...
    def _generate_fallback_summary(
        ...
        if bottlenecks:
            ...
            for i, bottleneck in enumerate(bottlenecks[:3], 1):
                lines.append(
                    f"  {i}. {bottleneck['function']}: {bottleneck['total_time']}s ({bottleneck['percentage']}%)"
                )
