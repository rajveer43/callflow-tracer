...
class TrendAnalyzer:
    ...
    def _analyze_function_trend(
        self, func_key: str, measurements: List[Tuple[str, float]]
    ) -> FunctionTrend:
    ...
    def _calculate_trend(self, values: List[float]) -> Tuple[str, float]:
        ...
        ss_res = sum(
            (y[i] - (slope * x[i] + (y_mean - slope * x_mean))) ** 2 for i in range(n)
        )
    ...
    def _generate_forecast(
        self, degrading: List[FunctionTrend], improving: List[FunctionTrend]
    ) -> Dict[str, Any]:
        """Generate forecast."""
        forecast = {
            ...
            forecast["degrading_functions_forecast"].append(
                {
                    ...
                    "current_value": (
                        func.measurements[-1][1] if func.measurements else 0
                    ),
