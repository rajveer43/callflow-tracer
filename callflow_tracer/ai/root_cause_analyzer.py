...
class RootCauseAnalyzer:
    ...
    def analyze(
        self, graph, issue_type: str = "performance", threshold: Optional[float] = None
    ) -> Dict[str, Any]:
        """
        ...
        if self.provider:
            llm_insights = self._get_llm_insights(
                root_causes, impact_analysis, nodes, issue_type
            )
    ...
    def _extract_nodes(self, graph) -> Dict[str, Dict[str, Any]]:
        ...
        for node_id, node in graph.nodes.items():
            nodes[node_id] = {
                ...
                "avg_time": (
                    node.total_time / node.call_count if node.call_count > 0 else 0
                ),
    ...
    def _build_dependency_graph(
        self, nodes: Dict, edges: List[Tuple]
    ) -> Dict[str, Dict]:
    ...
    def _identify_slow_nodes(
        self, nodes: Dict, threshold: Optional[float]
    ) -> List[str]:
    ...
    def _trace_root_causes(
        self, problematic: List[str], dependencies: Dict, nodes: Dict
    ) -> List[Dict[str, Any]]:
        """
        ...
        for problem_node in problematic:
            ...
            upstream_path = self._traverse_upstream(
                problem_node, dependencies, nodes, problematic
            )
                ...
                root_causes.append(
                    {
                        ...
                        "confidence": self._calculate_confidence(
                            root, upstream_path, downstream_impact
                        ),
                    }
                )
        ...
        root_causes.sort(
            key=lambda x: (x["confidence"], x["total_impact_time"]), reverse=True
        )

        return root_causes

    def _traverse_upstream(
        self, node_id: str, dependencies: Dict, nodes: Dict, problematic: List[str]
    ) -> List[str]:
        ...
        while current and current not in visited:
            ...
            slowest_caller = max(
                problematic_callers, key=lambda c: nodes[c]["total_time"]
            )
            path.insert(0, current)
    ...
    def _calculate_downstream_impact(
        self, node_id: str, dependencies: Dict, nodes: Dict
    ) -> Dict[str, Any]:
    ...
    def _calculate_confidence(self, root: str, path: List[str], impact: Dict) -> float:
    ...
    def _analyze_impact(
        self, root_causes: List[Dict], dependencies: Dict, nodes: Dict
    ) -> Dict[str, Any]:
        ...
        for root in root_causes:
            impact = self._calculate_downstream_impact(
                root["node_id"], dependencies, nodes
            )
        ...
        return {
            ...
            "impact_percentage": (
                (len(total_affected) / len(nodes) * 100) if nodes else 0
            ),
        }

    def _get_llm_insights(
        self, root_causes: List[Dict], impact: Dict, nodes: Dict, issue_type: str
    ) -> Optional[str]:
...
Be concise and actionable."""
    ...
    def _prepare_llm_context(
        ...
        return {"root_causes_text": root_causes_text}
