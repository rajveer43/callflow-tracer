...
def compare_graphs(
    graph1: CallGraph, graph2: CallGraph, label1: str = "Before", label2: str = "After"
) -> dict:
    """
    ...
    for name in all_node_names:
        ...
        if n1 and n2:
            ...
            time_diff_pct = (
                (time_diff / n1["total_time"] * 100) if n1["total_time"] > 0 else 0
            )
    ...
    improvements = [
        n for n in node_comparisons if n["improvement"] and n["status"] == "modified"
    ]
    regressions = [
        n
        for n in node_comparisons
        if not n["improvement"] and n["status"] == "modified"
    ]
    ...
    return {
        ...
        "summary": {
            ...
            "nodes_removed": len(
                [n for n in node_comparisons if n["status"] == "removed"]
            ),
            "nodes_modified": len(
                [n for n in node_comparisons if n["status"] == "modified"]
            ),
...
def export_comparison_html(
    ...
    label1: str = "Before",
    label2: str = "After",
    title: str = "Call Graph Comparison",
) -> None:
    """
