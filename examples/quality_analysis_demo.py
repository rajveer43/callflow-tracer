...
def long_function_with_poor_maintainability():
    ...
    variance = (
        sum((x - average) ** 2 for x in transformed) / len(transformed)
        if transformed
        else 0
    )
...
def _format_result(data):
    """Format the result."""
    return {"values": data, "count": len(data), "sum": sum(data)}
...
def main():
    ...
    print(
        "  for m in metrics: print(f'{m.function_name}: {m.cyclomatic_complexity}')\""
    )
