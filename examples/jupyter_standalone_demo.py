...
def demo_performance_profiling():
    ...
    if mem_stats1:
        print(
            f"Memory: {mem_stats1['current_mb']:.2f}MB (peak: {mem_stats1['peak_mb']:.2f}MB)"
        )
    ...
    if mem_stats2:
        print(
            f"Memory: {mem_stats2['current_mb']:.2f}MB (peak: {mem_stats2['peak_mb']:.2f}MB)"
        )
...
def demo_combined_analysis():
    ...
    if stats_dict["cpu"]:
        print(
            f"CPU profile captured: {len(stats_dict['cpu']['profile_data'])} characters"
        )
...
def demo_ml_pipeline():
    ...
    export_html(graph, output_file, title="Demo 5: ML Pipeline", profiling_stats=stats)
    print(f"\n✓ Exported to {output_file}")
