...
class EcommerceAPI:
    ...
    def search_products(
        self, query: str, min_price: float = 0, max_price: float = float("inf")
    ) -> List[Product]:
        """
        ...
        for i in range(len(customer_orders)):
            for j in range(i + 1, len(customer_orders)):
                if customer_orders[i].id > customer_orders[j].id:
                    customer_orders[i], customer_orders[j] = (
                        customer_orders[j],
                        customer_orders[i],
                    )
    ...
    def _validate_order_items(self, items: List[Dict]) -> Dict:
        ...
        for item in items:
            ...
            item_total = product.price * quantity
            validated_items.append(
                {"product": product, "quantity": quantity, "subtotal": item_total}
            )
            subtotal += item_total
    ...
    def _get_promo_discount(self, promo_code: str) -> float:
        ...
        discounts = {"SAVE10": 0.9, "SAVE20": 0.8, "SAVE5": 0.95}
    ...
    def _calculate_shipping(self, subtotal: float, method: str) -> float:
        ...
        shipping_costs = {"standard": 5.99, "express": 15.99, "overnight": 29.99}
    ...
    def _create_order(self, customer_id: int, items: List[Dict], total: float) -> Order:
        """Create and save order."""
        order = Order(
            id=len(self.orders) + 1, customer_id=customer_id, items=items, total=total
        )
        order.status = "confirmed"
        self.orders.append(order)
        return order
...
def monitor_api_quality():
    ...
    try:
        ...
        process_order_metric = next(
            (m for m in complexity_metrics if m.function_name == "process_order"), None
        )
        ...
        process_order_refactored_metric = next(
            (
                ...
                if m.function_name == "process_order_refactored"
            ),
            None,
        )
        ...
        if process_order_metric and process_order_refactored_metric:
            ...
            print(
                f"  Cyclomatic Complexity: {process_order_metric.cyclomatic_complexity}"
            )
            ...
            print(
                f"  Cyclomatic Complexity: {process_order_refactored_metric.cyclomatic_complexity}"
            )
            ...
            improvement = (
                (
                    process_order_metric.cyclomatic_complexity
        ...
        if debt_indicators:
            ...
            high_debt = [
                d for d in debt_indicators if d.severity in ["Critical", "High"]
            ]
...
def simulate_performance_monitoring():
    ...
    for i in range(1, 11):
        ...
        print(
            f"Iteration {i:2d}: {len(api.orders):3d} orders, query time: {elapsed:.6f}s"
        )
