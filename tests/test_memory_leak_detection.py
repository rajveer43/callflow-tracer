...
def test_growth_pattern_detection():
    ...
    if growth_patterns:
        print(
            f"Average growth rate: {sum(p['growth_rate'] for p in growth_patterns) / len(growth_patterns):.2f} bytes/sec"
        )
...
def test_top_memory_consumers():
    ...
    for i, consumer in enumerate(consumers[:3]):
        print(
            f"  #{i+1}: {consumer['size_mb']:.2f} MB ({consumer['count']} allocations)"
        )
