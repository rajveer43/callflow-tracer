...
def test_color_schemes():
    ...
    for scheme in color_schemes:
        output_file = os.path.join(
            os.path.dirname(__file__), f"test_enhanced_color_{scheme}.html"
        )

        generate_flamegraph(graph, output_file, color_scheme=scheme, show_stats=True)
        ...
        assert (
            scheme in content or "colorScheme" in content
        ), f"Should reference {scheme}"
...
def test_search_enabled():
    ...
    generate_flamegraph(graph, output_file, search_enabled=True, show_stats=True)
...
def test_statistics_panel():
    ...
    generate_flamegraph(graph, output_file, show_stats=True, color_scheme="performance")
    ...
    checks = [
        (
            "Statistics" in content or "stats-panel" in content,
            "Statistics panel present",
        ),
        (
            "Total Functions" in content or "total_functions" in content,
            "Total functions shown",
        ),
        ("Total Calls" in content or "total_calls" in content, "Total calls shown"),
        (
            "Slowest Function" in content or "slowest_function" in content,
            "Slowest function shown",
        ),
    ]
...
def test_export_functionality():
    ...
    generate_flamegraph(graph, output_file, show_stats=True, search_enabled=True)
...
def test_performance_color_scheme():
    ...
    output_file = os.path.join(
        os.path.dirname(__file__), "test_enhanced_performance.html"
    )
    ...
    assert (
        "#4ecdc4" in content or "#ff6b6b" in content
    ), "Should have performance colors"
...
def test_min_width_threshold():
    ...
    for threshold in thresholds:
        generate_flamegraph(graph, output_file, min_width=threshold, show_stats=True)

        assert os.path.exists(
            output_file
        ), f"Should create file with threshold {threshold}"
...
def test_all_features_combined():
    ...
    output_file = os.path.join(
        os.path.dirname(__file__), "test_enhanced_all_features.html"
    )
