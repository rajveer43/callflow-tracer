...
from callflow_tracer.visualization.comparison import compare_graphs, export_comparison_html
...
def test_basic_comparison():
    ...
    print(f"\n📊 Comparison Summary:")
    print(
        f"  Time saved: {summary['time_saved']:.4f}s ({summary['time_saved_pct']:.1f}%)"
    )
...
def test_regression_detection():
    ...
    print(
        f"Time change: {summary['time_saved']:.4f}s ({summary['time_saved_pct']:.1f}%)"
    )
...
def test_custom_labels():
    ...
    comparison = compare_graphs(
        graph1, graph2, label1="Version 1.0", label2="Version 2.0"
    )
