...
def build_package(skip_deps: bool, skip_clean: bool) -> bool:
    ...
    # Use setuptools directly to avoid module-name collisions with build.py.
    if not run_command(
...
def check_package(skip_check: bool):
    ...
    # Check with twine
    dist_files = [str(path) for path in dist_dir.glob("*") if path.is_file()]
    if not run_command(
...
def main():
    ...
    parser.add_argument(
        "--skip-check", action="store_true", help="Skip twine validation check"
    )
    parser.add_argument(
